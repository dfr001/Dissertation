  ...
  for(i1 in c("Normal","Logistic","Laplace","t")){
    for(i2 in est.des$theta + c(0)){
      for(i3 in est.des$K + c(1.5)){
        ...
        save(res,file=paste0("file/path/here/Des-k",est.des$k,"-",est.des$f,"-Th",est.des$theta,"-K",est.des$K,"/",i1,"-theta-",i2,"-delta-",i3,".Rda"))
        cat("F:",i1,"\n")
        cat("theta:",i2,"\n")
        cat("delta:",i3,"\n")
        rm(i3);rm(res)
...
for(o1 in 1){
  ...
  main.dir <- paste0("file/path/here",est.des$k,"-",est.des$f,"-Th",est.des$theta,"-K",est.des$K,"/")
