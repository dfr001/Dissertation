# this file gets read for the simulations run in the terminal
# function to get arm size for srs statistic
# packages: rmutil, mvtnorm
grp.seq.mix <- function(num.stages=2,alpha,beta,rho,theta,delta,K=1,mu=0,sigma=1,shape=2,f="normal",B=1e5,method="sar",norm.approx=FALSE){
  m.start = 1
  k <- num.stages
  ...
  if(f=="normal"){
    ...
    dpsi <- function(x,arg1=0,arg2=1,lg=FALSE){dlogis(x,location=arg1,scale=arg2*sqrt(3)/pi,log=lg)}
    ...
    dpsi <- function(x,arg1=0,arg2=1,lg=FALSE){rmutil::dlaplace(x,m=arg1,s=arg2/sqrt(2),log=lg)}
    ...
    # f <- function(n,arg1=mu,arg2=shape){rgamma(n=n,shape=arg2,scale=arg1/arg2)}
    # ppsi <- function(x,arg1=mu,arg2=shape){pgamma(x,shape=arg2,scale=arg1/arg2)}
    # dpsi <- function(x,arg1=mu,arg2=shape,lg=FALSE){dgamma(x,shape=arg2,scale=arg1/arg2,log=lg)}
    f <- function(n,arg1=mu,arg2=shape){rgamma(n=n,shape=arg2,scale=arg1)}
    ppsi <- function(x,arg1=mu,arg2=shape){pgamma(x,shape=arg2,scale=arg1)}
    dpsi <- function(x,arg1=mu,arg2=shape,lg=FALSE){dgamma(x,shape=arg2,scale=arg1,log=lg)}
  }
  ...
  if(d!="gamma"){ # probs for location shift
    ...
    lambda <- 1/2
    # P(X<Y)
    pxy <- integrate(function(y){ppsi(y)*((1-theta)*dpsi(y)+theta*dpsi(y-K))},-Inf,Inf)$value
    # P(X1 < Y1, X1 < Y2)
    pxyy <- integrate(function(y){(1-(1-theta)*ppsi(y)-theta*ppsi(y-K))^2*dpsi(y)},-Inf,Inf)$value
    # P(X1 < Y1, X2 < Y2)
    pxxy <- integrate(function(y){(ppsi(y))^2*((1-theta)*dpsi(y)+theta*dpsi(y-K))},-Inf,Inf)$value
  } else { # probs for gamma distr multiplicative shift
    h1<-function(y,theta,K) {ppsi(y)*((1-theta)*dpsi(y) + theta*dpsi(y/delta)/delta)}
    gamval<-integrate(h1,-Inf,Inf,theta,K)$value
    h2<-function(y,theta,K) {((1-(1-theta)*ppsi(y)-theta*ppsi(y/delta))^2)*dpsi(y)} 
    xi1<-integrate(h2,-Inf,Inf,theta,K)$value-gamval^2
    h3<-function(y,theta,K) {(ppsi(y))^2*((1-theta)*dpsi(y) + theta*dpsi(y/delta)/delta)}
    xi2<-integrate(h3,-Inf,Inf,theta,K)$value-gamval^2
    lambda <- 1/2
    # P(X<Y)
    pxy <- integrate(function(y){ppsi(y)*((1-theta)*dpsi(y)+theta*dpsi(y/delta)/delta)},-Inf,Inf)$value
    # P(X1 < Y1, X1 < Y2)
    pxyy <- integrate(function(y){(1-(1-theta)*ppsi(y)-theta*ppsi(y/delta))^2*dpsi(y)},-Inf,Inf)$value
    # P(X1 < Y1, X2 < Y2)
    pxxy <- integrate(function(y){(ppsi(y))^2*((1-theta)*dpsi(y)+theta*dpsi(y/delta)/delta)},-Inf,Inf)$value
  }
  ...
  if(norm.approx==FALSE){
    if(k==1){ # simulate just one stage
      ...
      inc <- function(incr = 1, m = m.start){
        while(p>=pi2){
          ...
          for(i in 1:B){
            if(d!="gamma"){
              ...
              obs.alt <- NULL
              y <- sapply(runif(m),FUN = function(i){if(i<theta){f(n=1,arg1=delta,arg2=arg2)}else{f(n=1,arg2=arg2)}})
              obs.alt <- rbind(obs.alt,matrix(c(f(n=m,arg2=sigma),y),ncol=2,nrow=m,byrow=F))
            } else {
              obs <- matrix(f(n=k*2*m,arg1=mu,arg2=shape),ncol=2,nrow=(k*m),byrow=F)
              obs.alt <- NULL
              y <- sapply(runif(m),FUN = function(i){if(i<theta){f(n=1,arg1=delta*mu,arg2=shape)}else{f(n=1,arg1=mu,arg2=shape)}})
              obs.alt <- rbind(obs.alt,matrix(c(f(n=m,arg1=mu,arg2=shape),y),ncol=2,nrow=m,byrow=F))
            }
        ...
        } else { out.df <- data.frame(Stages=k,f=d,theta=theta,delta=delta,row.names = "")}
      ...
      incr.arm.size <- function(incr = 1, p = 1, m = m.start){
        while(p>=pi2[k]){
          ...
          for(i in 1:B){
            if(d!="gamma"){
              ...
              obs <- matrix(f(n=k*2*m,arg1=mu,arg2=shape),ncol=2,nrow=(k*m),byrow=F)
              ind <- sample(1:2,prob=c(1-theta,theta),size=k*m,replace = TRUE);ind
              mus <- c(mu,delta*mu)
              y <- f(n=k*m,arg1=mus[ind],arg2=shape)
              obs.alt <- matrix(c(f(n=k*m,arg1=mu,arg2=shape),y),ncol=2,nrow=(k*m),byrow=F)
            }
            for(i1 in 1:k){
              if(method=="rerank"){
                ...
                srs[i,i1] <- sum(rank(obs[((i1-1)*m+1):(i1*m),])[(n+1):(n+m)])
                srs.alt[i,i1] <- sum(rank(obs.alt[((i1-1)*m+1):(i1*m),])[(n+1):(n+m)])
              }
            }
          }
          # ts = test statistic
          ...
          if(method!="rerank" & std==TRUE){ # sar standardized
            ...
            ts <- t(apply(srs,1,FUN=function(x) (cumsum(x)/seq_along(x) - mu)/(sig/sqrt(seq_along(x)))))
            ts.alt <- t(apply(srs.alt,1,FUN=function(x) (cumsum(x)/seq_along(x) - mu)/(sig/sqrt(seq_along(x)))))
          }
          if(method!="rerank" & std==FALSE){ # sar not standardized
            ts <- t(apply(srs,1,FUN=function(x) (cumsum(x)/seq_along(x))))
            ts.alt <- t(apply(srs.alt,1,FUN=function(x) (cumsum(x)/seq_along(x))))
          }
        ...
        if(dim(s)[1]==0 || dim(s.alt)[1]==0){return("Unable to achieve design.")
          ...
          # return(list(r=r,a=c(a[1:(k-1)],r[k]),m=m,p.r=p.r,p.a=p.a,p.out=p.out,K=K,theta=theta))
          if(d!="gamma"){
            out.df <- data.frame(Stages=k,f=d,theta=theta,K=K,row.names = "")
          } else { out.df <- data.frame(Stages=k,f=d,theta=theta,delta=delta,row.names = "")}
          out <- list("Design Alternative"=out.df,
                      ...
                      "Type II Error"=round(c(c(p.a[1:(k-1)],p.out),sum(c(p.a[1:(k-1)],p.out))),4))
          names(out$`Type I Error`) <- names(out$`Type II Error`) <- c(paste("Stage",1:k),"Overall")
          return(out)
        }
      }
    ...
    if(k==1){ # fixed sample
      ...
      while(p>=pi2){
        ...
        v <- m*m*(m+m+1)/12
        r <- qnorm(1-pi1)
        p <- pnorm(r,mean=(m*(m*pxy + (m+1)/2) - mu)/sqrt(v),sd=m^2*(pxy*(1-pxy)+(m-1)*(pxyy-pxy^2+pxxy-pxy^2))/v)
      }
      ...
      while(p>=pi2[k]){
        ...
        if(method=="sar" & std=="TRUE"){
          ...
          # alt.var <- ((m*m/sqrt(m+m))^2*(xi1/lambda + xi2/(1-lambda)))/((1:k)*v)
          alt.cov <- outer(1:k,1:k,Vectorize(function(x,y){sqrt(x*y)/max(x,y)}))*alt.v
        }
        if(method=="rerank" & std=="TRUE"){ # this doesn't work yet, maybe don't even need it
          mu <- (1:k)*m*((1:k)*(m+m)+1)/2
          v <- (1:k)^2*m*m*((1:k)*(m+m)+1)/12
          null.mu <- rep(0,k)
          null.var <- rep(1,k)
          null.cov <- outer(1:k,1:k,Vectorize(function(x,y){(((min(x,y)*m)^2*(2*max(x,y)*m+1))/12)/(sqrt(v[x]*v[y]))}))
          alt.mu <- ((1:k)*m*((1:k)*m*gamval + ((1:k)*m+1)/2) - mu)/sqrt(v)
          # don't even use these alt.var variables
          alt.var <- (((1:k)*m)^2*(pxy*(1-pxy) + ((1:k)*m-1)*(pxyy-pxy^2) + ((1:k)*m-1)*(pxxy-pxy^2)))/v
          # from jeske/yao
          # alt.var2 <- (((1:k)^2*m*m/sqrt((1:k)*m+(1:k)*m))^2*(xi1/lambda + xi2/(1-lambda)))/(v)
          alt.cov <- outer(1:k,1:k,Vectorize(function(x,y){
            # ((min(x,y)*m)^2*(gamval*(1-gamval) + (max(x,y)*m-1)*(xi1) + (max(x,y)*m-1)*(xi2)))/sqrt(v[x]*v[y])
            ((min(x,y)*m)^2*(pxy*(1-pxy) + (max(x,y)*m-1)*(pxyy-pxy^2 + pxxy-pxy^2)))/sqrt(v[x]*v[y])
          }))
        }
        ...
        for(i in 2:k){ # uses mvtnorm package
          if(m>3){
            r[i] <- uniroot(f=function(x){as.numeric(mvtnorm::pmvnorm(lower=c(a[1:(i-1)],x),upper=c(r[1:(i-1)],Inf),mean=null.mu[1:i],sigma=null.cov[1:i,1:i])) - pi1[i]},
                            lower=0,upper=4)$root
          } else {
            r[i] <- optimize(f=function(x){(as.numeric(mvtnorm::pmvnorm(lower=c(a[1:(i-1)],x),upper=c(r[1:(i-1)],Inf),mean=null.mu[1:i],sigma=null.cov[1:i,1:i])) - pi1[i])^2},
                             lower=0,upper=4)$minimum
          }
          if(i!=k){
            if(m>3){
              a[i] <- uniroot(f=function(x){as.numeric(mvtnorm::pmvnorm(lower=c(a[1:(i-1)],-Inf),upper=c(r[1:(i-1)],x),mean=alt.mu[1:i],sigma=alt.cov[1:i,1:i])) - pi2[i]},
                              lower=-4,upper=4)$root
            } else {
              a[i] <- optimize(f=function(x){(as.numeric(mvtnorm::pmvnorm(lower=c(a[1:(i-1)],-Inf),upper=c(r[1:(i-1)],x),mean=alt.mu[1:i],sigma=alt.cov[1:i,1:i])) - pi2[i])^2},
        ...
        p <- as.numeric(mvtnorm::pmvnorm(lower=c(a[1:(k-1)],-Inf),upper=c(r[1:(k-1)],r[k]),mean=alt.mu[1:k],sigma=alt.cov[1:k,1:k]))
        ...
        for(i in 2:k){
          if(i!=k){ # uses mvtnorm package
            p.r[i] <- mvtnorm::pmvnorm(lower=c(a[1:(i-1)],r[i]),upper=c(r[1:(i-1)],Inf),mean=null.mu[1:i],sigma=null.cov[1:i,1:i])
            p.a[i] <- mvtnorm::pmvnorm(lower=c(a[1:(i-1)],-Inf),upper=c(r[1:(i-1)],a[i]),mean=alt.mu[1:i],sigma=alt.cov[1:i,1:i])
          } else {
            p.r[i] <- mvtnorm::pmvnorm(lower=c(a[1:(i-1)],r[i]),upper=c(r[1:(i-1)],Inf),mean=null.mu[1:i],sigma=null.cov[1:i,1:i])
            p.a[i] <- mvtnorm::pmvnorm(lower=c(a[1:(i-1)],-Inf),upper=c(r[1:(i-1)],r[i]),mean=alt.mu[1:i],sigma=alt.cov[1:i,1:i])
          }
          rm(i)
        }
      }
